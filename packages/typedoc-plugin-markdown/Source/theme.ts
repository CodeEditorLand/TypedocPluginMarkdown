...
import type { HasOwnDocument, TemplateMapping } from "./models";
...
export class MarkdownTheme extends Theme {
	...
	buildUrls(
		...
		if (mapping) {
			...
			if (!reflection.url || !URL_PREFIX.test(reflection.url)) {
				...
				const friendlyFragments = reflection
					.getFriendlyFullName()
					.split(".");
				...
				if (
					reflection.groups?.some((group) =>
						group.children.some((child) =>
							Boolean(this.mappings[child.kind]),
						),
					)
				) {
	...
	getMappingsMap(
		...
		if (
			hasOwnDocument.includes("all") ||
			hasOwnDocument.includes("function")
		) {
		...
		if (
			hasOwnDocument.includes("all") ||
			hasOwnDocument.includes("variable")
		) {
