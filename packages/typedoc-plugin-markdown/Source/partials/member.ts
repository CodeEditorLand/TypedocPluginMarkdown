...
export function member(
	...
	if (context.getOption("namedAnchors")) {
		md.push(
			`<a id="${reflection.anchor}" name="${reflection.anchor}"></a>`,
		);
	}
		...
		if (context.getOption("embedHeadingsInCodeBlock")) {
			md.push(
				heading(
					headingLevel,
					backTicks(
						context.partials.reflectionTitle(reflection, false),
					),
				),
			);
		} else {
			md.push(
				heading(
					headingLevel,
					context.partials.reflectionTitle(reflection),
				),
			);
		}
	}
	...
	} else if (reflection.hasGetterOrSetter()) {
	...
	} else if (reflection instanceof ReferenceReflection) {
