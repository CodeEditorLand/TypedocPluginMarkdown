	...
	if (reflection.sources) {
		...
		reflection.sources.forEach((source) => {
			if (source.url) {
				sources.push(
					link(
						`${escapeChars(source.fileName)}:${source.line}`,
						source.url,
					),
				);
			} else {
...
const typeAndParent = (
	...
	if (props) {
		if ("elementType" in props) {
			...
			if (props.reflection instanceof SignatureReflection) {
			...
			} else if (props.reflection.parent?.url) {
				md.push(
					getUrl(
						props.reflection.parent.name,
						props.reflection.parent.url,
				...
				if (props.reflection.url) {
					md.push(
						getUrl(props.reflection.name, props.reflection.url),
					);
				}
			}
